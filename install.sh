...
case "$MODEL" in
    tiny|base|small|medium|large-v3|large-v3-turbo|large-v3-turbo-q5_0) ;;
    *) die "неизвестная модель: $MODEL" ;;
esac
...
if [ ! -s "$MODEL_FILE" ]; then
    case "$MODEL" in
        tiny) MSZ=75 ;; base) MSZ=142 ;; small) MSZ=466 ;; medium) MSZ=1462 ;;
        large-v3) MSZ=2951 ;; large-v3-turbo) MSZ=1549 ;; large-v3-turbo-q5_0) MSZ=547 ;;
    esac
    say "скачиваю модель ggml-$MODEL.bin (~$MSZ МБ)"
